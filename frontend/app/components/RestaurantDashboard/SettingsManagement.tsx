...
import { useUpdateRestaurantInfoMutation } from '../../store/services/restaurant';
import { RootState } from '@/app/store/store';
import { ImageProps, Restaurant } from '@/app/types/restaurant.types';
...
const PreviewImage = ({ src, alt, className }: PreviewImageProps) => {
  if (src && (src.startsWith('data:') || src.startsWith('blob:'))) {
    return <img src={src} alt={alt} width={100} height={100} className={className} />;
  }
  return <img src={src} alt={alt} width={400} height={192} className={className} />;
};

// Type local pour le formulaire des settings du restaurant
...
const SettingsManagement = () => {
  ...
  // Mutation RTK Query — isLoading et error viennent directement de la mutation
  const [updateRestaurantInfo, { isLoading, isError, error }] = useUpdateRestaurantInfoMutation();
  ...
  const [restaurantData, setRestaurantData] = useState<RestaurantFormData>({
    ...
    openingHours: (user?.restaurant?.openingHours as RestaurantFormData['openingHours']) || defaultOpeningHours,
    baseFee: user?.restaurant?.baseFee ?? 0,
  ...
  const getPreviewUrl = (type: string): string | undefined => {
    if (previewUrls[type as keyof ImageProps]) {
      return previewUrls[type as keyof ImageProps] ?? undefined;
    }
    const dbImage = user?.restaurant?.[type as keyof Restaurant];
  ...
  const handleSaveChanges = async () => {
    ...
    if (selectedFiles.img) form.append('img', selectedFiles.img);
    if (selectedFiles.coverImg) form.append('coverImg', selectedFiles.coverImg);

    try {
      await updateRestaurantInfo({ id: user?.restaurant?._id ?? '', form }).unwrap();
    } catch (error) {
      console.error('Erreur lors de la sauvegarde:', error);
    }
  };

  // Synchronise le formulaire quand user change (ex: après un onQueryStarted de authMe)
  useEffect(() => {
    if (user?.restaurant) {
      setRestaurantData({
        name: user.restaurant.name || 'Mon Restaurant',
  ...
  useEffect(() => {
    return () => {
      if (previewUrls.img?.startsWith('blob:')) URL.revokeObjectURL(previewUrls.img);
      if (previewUrls.coverImg?.startsWith('blob:')) URL.revokeObjectURL(previewUrls.coverImg);
    };
  }, [previewUrls]);
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'General' && (
        <div className="bg-white rounded-lg shadow p-6">
          ...
          {isError && <div className="bg-red-100 p-2 font-weight mb-4">{errorMessage}</div>}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (file) {
                        ...
                        setPreviewUrls(prev => ({ ...prev, img: blobUrl }));
                        setSelectedFiles(prev => ({ ...prev, img: file }));
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (file) {
                        const blobUrl = URL.createObjectURL(file);
                        setPreviewUrls(prev => ({ ...prev, coverImg: blobUrl }));
                        setSelectedFiles(prev => ({ ...prev, coverImg: file }));
